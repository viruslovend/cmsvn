...
function check_user_permissions($request, $actionName = NULL, $id = NULL)
{
    ...
    foreach ($crudPermissionsMap as $permission => $methods)
    {
        ...
        if (in_array($method, $methods) && isset($classesMap[$controller]))
        {
			...
			// Page check
			if ($className == 'page' && in_array($method, ['edit', 'update', 'destroy', 'restore', 'forceDestroy']))
            {
                $id = !is_null($id) ? $id : $request->route('page');

                // if the current user has not update-others-page/delete-others-page permission
                // make sure she/he only modify his/her own page
                if ( $id &&
                    (!$currentUser->can('update-others-page') || !$currentUser->can('delete-others-page')) )
                {
                    $page = \App\Page::withTrashed()->find($id);
                    if ($page->author_id !== $currentUser->id) {
                        return false;
                    }
                }
            }
